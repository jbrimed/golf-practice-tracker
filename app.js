...
function updateStartButton() {
    ...
    if(btn) {
        ...
        if (selectedDrillIds.size === 0) btn.classList.add("opacity-50", "cursor-not-allowed");
...
function triggerAutoSave() {
    ...
    selectedDrillIds.forEach(id => {
        const scoreInput = document.querySelector(`.drill-score-input[data-id="${id}"]`);
        const noteInput = document.querySelector(`textarea[data-note-id="${id}"]`);
        drillData[id] = { score: scoreInput ? scoreInput.value : "", note: noteInput ? noteInput.value : "" };
    });
    saveDraft({ date: $("session-date")?.value, skills: Array.from(selectedSkills), drills: Array.from(selectedDrillIds), drillData });
}

// ----------------------
// HISTORY / ANALYTICS (Placeholder for brevity - same as previous, verified working)
// ----------------------
async function renderHistory() {
    ...
    sessions.forEach(s => {
        ...
        div.addEventListener("click", (e) => { if(!e.target.classList.contains("del-hist-btn")) {
